...
export default function AppointmentItem({appointment}) {
    return (
        <tr>
            <td>
                <Link href={appointment.patient ? route('patients.show', {user: appointment.patient?.id}) : '#'} className='link'>{appointment.patient?.full_name}</Link>
            </td>
            <td>
                <Link href={appointment.doctor ? route('practitioners.show', {user: appointment.doctor?.id}) : "#"} className='link' >{appointment.doctor?.full_name}</Link>
            </td>
            <TableLink className='text-center' href={route('appointments.show', {appointment: appointment.id})}>{appointment.type}</TableLink>
            ...
            <TableLink href={route('appointments.show', {appointment: appointment.id})}>{appointment.status}</TableLink>
